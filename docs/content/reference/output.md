...
describes what users get. Pick a format with `-o`, or let ncbiassembly choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
ncbiassembly <command> -o table   # aligned columns for reading
ncbiassembly <command> -o jsonl   # one JSON object per line, for piping
ncbiassembly <command> -o json    # a single JSON array
ncbiassembly <command> -o csv     # spreadsheet friendly
ncbiassembly <command> -o tsv     # tab-separated
ncbiassembly <command> -o url     # just the URL column
ncbiassembly <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
ncbiassembly <command> --fields id,title,url
```
...
```bash
ncbiassembly <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
ncbiassembly <command>            # a table, because this is a terminal
ncbiassembly <command> | wc -l    # JSONL, because this is a pipe
```
